...
test('stake chef will not hash an empty array', t => {
  ...
  t.equal(stakeChef(noStats).hash, '-', 'No object resturns blank')
  t.equal(stakeChef(noStatsData).hash, '-', 'Object with no stats returns blank')
  t.equal(stakeChef(noNodes).hash, '-', 'No nodes array returns blank')
  t.equal(stakeChef(emptyNodes).hash, '-', 'Empty nodes array returns blank')
})
